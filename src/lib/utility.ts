...
export const getCommonAnime = (
                ...
                list.entries.forEach((entry) => {
                    entry.media.recommendations?.nodes.forEach((node) => {
                        if (
                            ...
                            const recommendedMediaEntry =
                                recommendedMediaCount.get(media);
    ...
    const recommendedMediaCollection = Array.from(
        recommendedMediaCount.values()
    )
        .map((media) => {
    ...
    const sortedCommonMediaCollection = commonMediaCollection.sort((a, b) => {
        return (
            ...
            b.media.meanScore - a.media.meanScore
        );
    });
